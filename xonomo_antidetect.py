...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Base: Titan v8 logic (canvas 100% preserved exactly)
New in v8:
...
PROFILES_DIR  = os.path.join(BASE_DIR, "xonomo_profiles")
DB_FILE       = os.path.join(BASE_DIR, "xonomo_db.json")
SESSION_FILE  = os.path.join(BASE_DIR, "xonomo_session.json")
...
def _get_clean_sa() -> dict:
    ...
    if "-----BEGIN PRIVATE KEY-----" not in pk:
        raise ValueError("[XONOMO] Private key malformed.")
    sa["private_key"] = pk
    return sa
...
def get_device_code() -> str:
    try:
        return hashlib.sha256(
            f"xonomo-{socket.gethostname()}-{_uuid.getnode()}".encode()
        ).hexdigest()[:10].upper()
    except Exception:
        return "UNKNOWN0000"
...
def load_meta(pid):
    mp=os.path.join(PROFILES_DIR,pid,"xonomo_meta.json")
    try:
...
def save_meta(pid,fp,proxy,tz,name="",serial=0,lang_mode="default",
    ...
    with open(os.path.join(pdir,"xonomo_meta.json"),"w") as f:
        json.dump({
...
class BrowserThread(QThread):
    ...
    def _cookies_file(self) -> str:
        return os.path.join(PROFILES_DIR, self.pid, "_xonomo_cookies.json")
...
console.log('%c[XONOMO v8] MAC:{mac} LAN:{fl} WebRTC:{webrtc_mode}','color:#28a745;font-weight:bold;');
}})();
"""
...
class XonomoAntiDetect(QMainWindow):
    def __init__(self):
        super().__init__()
    ...
    def _build_ui(self):
        ...
        ll.addStretch()
        ttl=QLabel("🛡️  XONOMO Anti-Detect Browser")
    ...
    def _all_cookie_req(self, pid):
        ...
        if t and t.isRunning():
            ...
            cf = os.path.join(PROFILES_DIR, pid, "_xonomo_cookies.json")
            if os.path.exists(cf):
                try:
...
if __name__ == "__main__":
    ...
    win=XonomoAntiDetect(); win.show(); sys.exit(app.exec())